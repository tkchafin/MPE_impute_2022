...
c = treeCl.Collection(input_dir='fasta_test', file_format='fasta', trees_dir="out_test")

# geodesic distances for tree set
dm = c.get_inter_tree_distances('geo',show_progress=False)
dm.to_csv("pairwise_geodesic.tsv", sep="/t", quoting=None)
...
embedding.df.to_csv("embedding.tsv", sep="\t", quoting=None)
...
k, eigenvals,  eigenvecs = eigenDecomposition(affinity_matrix)
